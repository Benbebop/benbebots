local bin = include( "hostlink/pack.lua" )
...
do
	...
	http.Fetch(	"https://api.github.com/repos/Benbebop/Benbebot/contents/scr/lua/srcds/signals.json", function( body )
		coroutine.resume( thread, util.JSONToTable(body) )
	end,function( message )
		error( message )
	end)
	
	local github = coroutine.yield()
	
	if github.encoding ~= "base64" then error( "github returned non base64 encoded content (" .. github.encoding .. ")" ) end
	
	singalIndex = util.JSONToTable( util.Base64Decode( github.content ) )
end
...
function pipe.sendSignal( self, signal, callback, ... )
	...
	local output = file.open( "pseudopipe/pipe_1.dat", "ab", "DATA" )
