...
do -- game server
	...
	if los.type() == "win32" then -- garrys mod
		...
		local function autocompleteFromTable(tbl, str, indexFunc)
			local n, results = 0, {}
			for _,v in ipairs(tbl) do
				local value = indexFunc(v)
				local found = value:find(str, nil, true)
				if found then
					table.insert(results, {value, value})
					n = n + 1
				end
				if n >= MAX_SEARCH_RESULTS then
					break
				end
			end
			return results
		...
		local collections = json.parse(fs.readFileSync(pathJoin(GARRYSMOD_DIR, "garrysmod/data/collections.json")) or "") or {}
		local collectionCache = setmetatable({}, {__mode = "v"})
		local function getCollection(str)
			local result = collectionCache[str]
			if result then return result end
			
			for _,v in ipairs(collections) do
				local s, e = v.name:find(str, nil, true)
				if s == 1 and e == #v.name then
					result = v
					break
				end
			end
			...
			if result then
				collectionCache[str] = result
				return result
			end
		end
		
		local mapCache = {}
		local function getMaps(collectionId)
			local prev = mapCache[collectionId]
			...
			return mapCache[collectionId]
		end
		
		cmd:used({"gmod","gamemode"}, function(interaction, args) --TODO
			
		end)
		
		cmd:autocomplete({"gmod","addon"}, function(interaction, args, _, focused)
			if focused ~= "gamemode" then return end
			
			return autocompleteFromTable(collections, args.gamemode, function(v) return v.name end)
		end)
		...
		cmd:autocomplete({"gmod","start"}, function(interaction, args, _, focused)
			if focused == "gamemode" then
				return autocompleteFromTable(collections, args.gamemode, function(v) return v.name end)
			end
			local collection = getCollection(args.gamemode)
			if not collection then return {} end
			
			return autocompleteFromTable(getMaps(collection.id), args.map, function(v) return v end)
		end)
		...
		cmd:used({"gmod","start"}, function(interaction, args)
			...
			local collection = getCollection(args.gamemode) or {}
			
			-- map
			
			local maps = getMaps(collection.id) or {}
			
			local map 
			for _,v in ipairs(maps) do
				local s, e = v:find(str, nil, true)
				if s == 1 and e == #v.name then
					map = v
					break
				end
			...
			addArg(args, "+gamemod", collection.gamemode or "sandbox")
			addArg(args, "+map", map or "gm_construct")
